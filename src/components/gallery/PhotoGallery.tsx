...
export const PhotoGallery = () => {
  // 1. Changed length to 9
  const photos = Array.from({ length: 9 }).map((_, i) => ({ 
    id: i, 
    rotation: Math.random() * 8 - 4, 
    color: i % 2 === 0 ? 'bg-blue-50' : 'bg-red-50' 
  }));

  return (
    <section className="min-h-screen py-24 bg-white border-t border-gray-50 flex flex-col items-center">
      <div className="text-center mb-16">
        <h2 className="text-5xl font-playfair mb-4">For you, Dad.</h2>
        <p className="text-gray-500 font-inter">A collection of moments and memories.</p>
      </div>
      
      {/* 2. Changed grid classes to max out at md:grid-cols-3 */}
      <div className="max-w-6xl w-full px-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-12">
        {photos.map((photo) => (
          <motion.div 
            key={photo.id} 
