...
export const Pong = ({ onBeat }: { onBeat?: () => void }) => {
  ...
  useEffect(() => {
    ...
    // 1. Changed base speed from 4 to 2.5 to make it slower
    let ball = { x: 400, y: 200, dx: 2.5, dy: 2.5, radius: 8 };
    ...
    const update = () => {
      ...
      if ((ball.x < p1.width && ball.y > p1.y && ball.y < p1.y + p1.height) ||
        ...
        // 2. Toned down bounce acceleration multiplier from -1.1 to -1.05
        ball.dx *= -1.05;
      }
    ...
    const resetBall = () => {
      ...
      // 3. Changed reset values from 4 to 2.5 to match the new speed profile
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-6">
      <div className="flex justify-between w-full max-w-lg text-4xl font-playfair text-accent">
        <span>{score.p1}</span>
        <span>{score.p2}</span>
      </div>
      <div className="relative">
        ...
        {winner && (
          <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-white">
            ...
            <button 
              onClick={() => { setScore({ p1: 0, p2: 0 }); setWinner(null); }} 
